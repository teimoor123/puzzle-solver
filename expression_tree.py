...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        """
        copied = self.copy()
        copied.substitute(lookup)
        if isinstance(copied._root, int):
            return copied._root
        elif copied._root == OP_ADD:
            rslt = 0
            for item in copied._subtrees:
                if isinstance(item._root, int):
                    rslt += item._root
                else:
                    rslt += item.eval(lookup)
        else:
            rslt = 1
            for item in copied._subtrees:
                if isinstance(item._root, int):
    ...
    def __str__(self) -> str:
        ...
        if self.is_empty():
            return '()'
        if isinstance(self._root, int) or self._root.isalpha():
            ...
            for subtree in self._subtrees:
                rslt = rslt[:i] + str(subtree) + ' ' + str(op) + ' ' + rslt[i:]
                i += len(str(subtree)) + 3
        return rslt[:-4] + rslt[-1]
    ...
    def __eq__(self, other: ExprTree) -> bool:
        ...
        False
        """
        return self._root == other._root and self._subtrees == other._subtrees
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    if isinstance(values[0][0], int) or str(values[0][0]).isalpha():
        return ExprTree(values[0][0], [])
    t = ExprTree(values[0][0], [])
    q = Queue()
    q.enqueue(t)
