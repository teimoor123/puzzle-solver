...
class SudokuPuzzle(Puzzle):
        ...
        for i in range(self._n):
            for j in range(self._n):
                if self._grid[i][j] == EMPTY_CELL:
                    set_used = self._row_set(i) | self._column_set(j) | \
                        self._subsquare_set(i, j)
                    lst.append(set_used)
        for lst2 in lst:
            if all(item in lst2 for item in self._symbol_set):
                return True
        return False
    ...
    def has_unique_solution(self) -> bool:
        ...
        seen = set()
        d_solver = DfsSolver()
        while d_solver.solve(self, seen) != [] and\
                d_solver.solve(self, seen) != [self]:
            if str(d_solver.solve(self, seen)[-1]) not in seen:
                seen = seen.union({str(d_solver.solve(self, seen)[-1])})
        if len(seen) != 1:
            return False
        return True
