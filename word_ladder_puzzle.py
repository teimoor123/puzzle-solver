...
class WordLadderPuzzle(Puzzle):
    ...
    def __eq__(self, other: WordLadderPuzzle) -> bool:
        ...
        True
        """
        return self.to_word == other.to_word and self.from_word ==\
            other.from_word and self.word_set == other.word_set
    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        for i in range(len(self.from_word)):
            for j in range(len(LETTERS)):
                word = self.from_word[:i] + LETTERS[j] + self.from_word[i + 1:]
                if word in self.word_set and word != self.from_word:
                    lst.append(WordLadderPuzzle(word, self.to_word,
                                                self.word_set))
        lst.sort()
        return lst
